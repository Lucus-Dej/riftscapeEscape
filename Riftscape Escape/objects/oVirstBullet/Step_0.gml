...
if (oItemManager.hasConRift) {
	if (linkTimer > 0) {
		...
		collision_circle_list(x, y, 128, oVirstBullet, false, true, nearbyBullets, true);
...
if (isTurret && !firedFromTurret && turretApplied) {
		turretDelay--;
		var tar = instance_nearest(x, y, oTruePlayer);
		if (instance_exists(tar)) {
			...
			if (turretDelay <= 0 && instance_exists(oTruePlayer) && line == noone) {
				var f = bulletFire(x, y, tarDir, bulletSpeed*0.7, damage*0.3, object_index, self);
...
speedBonus = abs(speed*0.08-bulletSpeed*0.06-damage*0.1);